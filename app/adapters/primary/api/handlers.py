...
from app.core.domain.base.exceptions import BaseDomainError
...
from app.core.services.check.exceptions import BaseCheckServiceError
...
from app.adapters.primary.api.version_1.authorization.exceptions import BaseAuthorizationApiError, UserIsNotAdminError
...
def registering_exception_handlers(app: FastAPI):
    ...
    @app.exception_handler(EntityNotExistsError)
    async def _(request: Request, exc: EntityNotExistsError):
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND,
            content={
                "detail": exc.message,
