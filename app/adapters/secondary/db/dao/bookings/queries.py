...
from app.adapters.secondary.db.dao.bookings.helpers import (
...
async def get_services(
    ...
    query = select(ServiceVarietiesModel).select_from(ServiceVarietiesModel).distinct(ServiceVarietiesModel.id)

    if only_for_hotels_and_only_for_rooms.only_for_hotels:
        query = query.join(HotelsServicesModel, HotelsServicesModel.service_variety_id == ServiceVarietiesModel.id)
    elif only_for_hotels_and_only_for_rooms.only_for_rooms:
        query = query.join(RoomsServicesModel, RoomsServicesModel.service_variety_id == ServiceVarietiesModel.id)
...
async def get_premium_levels(
    ...
    if hotel_id is not None:
        query = (
            query.join(RoomsModel, RoomsModel.premium_level_id == PremiumLevelVarietiesModel.id)
            .join(
                HotelsModel,
                RoomsModel.hotel_id == HotelsModel.id,
            )
        ...
        query = query.join(RoomsModel, RoomsModel.premium_level_id == PremiumLevelVarietiesModel.id)
...
async def get_rooms(
    ...
    query = (
        ...
        .join(HotelsModel, RoomsModel.hotel_id == HotelsModel.id)
        .join(
